...
def main():
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT id, gercek_baslik, gercek_yazarlar FROM orijinal_metadatalar")
    rows = cursor.fetchall()
    
    data_dict = {str(r[0]): {'Orijinal Başlık': r[1], 'Orijinal Yazarlar': r[2]} for r in rows}
    ...
    for filename in os.listdir(tei_dir):
        ...
        # Already annotated?
        if "<titlePart>" in text and "<docAuthor>" in text:
            success_title += 1
            success_author += 1
            continue
        ...
        # 1. Title Annotation
        if true_title and "<titlePart>" not in text:
        ...
        # 2. Author Annotation
        if true_authors and "<docAuthor>" not in text:
