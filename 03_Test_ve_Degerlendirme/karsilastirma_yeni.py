...
def karsilastir_grobid_ve_trdizin(grobid_klasoru, json_dosyasi, cikti_klasoru, db_adi="tubitak_makaleler.db"):
    ...
    for row in rows:
        ...
        mapped_dict = {
            ...
            # Makaleler iki dilli: GROBID Türkçe "Öz" yerine İngilizce "Abstract"ı
            # çıkardığında skor haksız yere 0 oluyordu. Artık iki dil de referans.
            "Gerçek Başlık EN": row_dict.get("gercek_baslik_en") or "",
            "Gerçek Özet EN": row_dict.get("gercek_ozet_en") or "",
            "Gerçek Anahtar Kelimeler EN": row_dict.get("gercek_anahtar_kelimeler_en") or "",
    ...
    for i, xml_yolu in enumerate(xml_dosyalari):
        ...
        with open(xml_yolu, "r", encoding="utf-8") as f:
            ...
            # Başlık
            # DİKKAT: yedek olarak soup.find('title') kullanılmamalı; TEI'de ilk
            # <title> çoğu zaman <title level="j"> yani DERGİ adı oluyordu ve
            # dergi adı makale başlığı sanılıyordu (1435 dosyada 27 vaka).
                    ...
                    if isimler:
                        for isim in isimler:
                            ...
                            if text:
                                ...
                                # DİKKAT: eskiden substring eşleşmesi yapılıyordu ve
                                # "A(dr)iatik", "San(dr)a", "Gün(gör)" gibi GERÇEK
                                # yazarlar eleniyordu (6280 ismin 252'si). Artık
                                # kelime bazlı: sadece tamamı unvandan ibaret girdiler atılır.
                                yasakli_kelimeler = {"prof", "dr", "doç", "doc", "uzman", "uzm",
                                                     "araş", "aras", "gör", "gor", "yrd", "öğr", "ogr"}
                                parcalar = {p.strip(".,;:").lower() for p in text.split() if p.strip(".,;:")}
                                sadece_unvan = bool(parcalar) and parcalar <= yasakli_kelimeler
                                if not sadece_unvan:
                                    authors.append(text)
        ...
        if orijinal:
            ...
            # İki dilli referans: TR ve EN karşılıklarından hangisi daha iyi
            # eşleşiyorsa o alınır (GROBID hangi dildeki bloğu çıkardıysa).
            def en_iyi_skor(grobid_metin, tr_metin, en_metin):
                adaylar = [advanced_token_similarity(grobid_metin, m)
                           for m in (tr_metin, en_metin) if m]
                adaylar = [a for a in adaylar if a[0] is not None]
                if not adaylar:
                    return None, None
                return max(adaylar, key=lambda x: x[0])

            orijinal_baslik_en = str(orijinal.get('Gerçek Başlık EN') or '')
            orijinal_ozet_en = str(orijinal.get('Gerçek Özet EN') or '')
            orijinal_kelimeler_en = str(orijinal.get('Gerçek Anahtar Kelimeler EN') or '')

            baslik_recall, baslik_precision = en_iyi_skor(grobid_baslik, orijinal_baslik, orijinal_baslik_en)
            ozet_recall, ozet_precision = en_iyi_skor(grobid_ozet, orijinal_ozet, orijinal_ozet_en)
            ...
            kelime_recall, kelime_precision = en_iyi_skor(grobid_keywords, orijinal_kelimeler, orijinal_kelimeler_en)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--grobid_klasoru", default=os.path.join(BASE_DIR, "makaleler_test", "grobid_xml_yeni_model"))
